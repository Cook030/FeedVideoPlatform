...
5. RabbitMQ 投递失败时，接口应该怎么处理？
   ...
   答：对已启用异步管线的点赞收藏，投递失败意味着后续持久化存在风险。GCFeed 在 `setActionAsync` 里捕获发布错误并回退到 `setActionSync`，由接口同步写入 MySQL 和计数，用户请求不会失败，行为事实也不会丢。视频发布和曝光链路目前只记录发布错误，尚未实现补偿。更完整的方案是本地 outbox 表：在同一事务里写业务数据和事件表，再由 relay 投递，保证事件最终一致；GCFeed 尚未采用。核心原则是用户看到的状态和最终事实之间必须有补偿路径。

6. Worker ACK/NACK 的策略怎么设计？

   答：处理成功后 ACK。JSON 解析失败属于永久性错误，直接 `Nack(requeue=false)` 丢弃，重试没有意义。业务处理失败时不能简单地 `Nack(requeue=true)`：requeue 投递的是原始消息，重试次数无法累加，一条永远失败的消息会被无限重投并堵死队列。GCFeed 的做法是重新发布同一条消息并把 `x-retry-count` 加一，再 ACK 原消息；达到上限（默认 3 次）后 `Nack(requeue=false)`，由队列的死信交换机转入 `<queue>.dlq`。指标会分别记录 consume 与 dead_letter 结果。
